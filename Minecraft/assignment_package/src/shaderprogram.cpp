...
void ShaderProgram::setUnifIntVec2(std::string name, int i, int j) {
    useMe();
    try {
        int handle = m_unifs.at(name);
        if(handle != -1) {
            context->glUniform2i(handle, i, j);
        }
    }
    catch(std::out_of_range &e) {
        std::cout << "Error: could not find shader variable with name " << name << std::endl;
...
void ShaderProgram::drawTransparent(Drawable &d) {
    if(d.elemCount(IDX_TRANS) < 0) {
        throw std::invalid_argument(
            "Trans - Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()."
            );
    }
    useMe();

    int handle;
    if ((handle = m_attribs["vs_Pos"]) != -1 && d.bindBuffer(DATA_TRANS)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)0);
    }

    if ((handle = m_attribs["vs_Nor"]) != -1 && d.bindBuffer(DATA_TRANS)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)sizeof(glm::vec4));
    }

    if ((handle = m_attribs["vs_Col"]) != -1 && d.bindBuffer(DATA_TRANS)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)(2*sizeof(glm::vec4)));
    }

    d.bindBuffer(IDX_TRANS);
    context->glDrawElements(d.drawMode(), d.elemCount(IDX_TRANS), GL_UNSIGNED_INT, 0);
...
void ShaderProgram::drawOpaque(Drawable &d) {
    if(d.elemCount(IDX_OPQ) < 0) {
        throw std::invalid_argument(
            "Opaque - Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()."
            );
    }
    useMe();

    int handle;
    if ((handle = m_attribs["vs_Pos"]) != -1 && d.bindBuffer(DATA_OPQ)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)0);
    }

    if ((handle = m_attribs["vs_Nor"]) != -1 && d.bindBuffer(DATA_OPQ)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)sizeof(glm::vec4));
    }

    if ((handle = m_attribs["vs_Col"]) != -1 && d.bindBuffer(DATA_OPQ)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)(2*sizeof(glm::vec4)));
    }
