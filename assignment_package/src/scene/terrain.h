...
private:
    ...
    float expansionWaitTime = 0.f;
    glm::vec3 terrainGenerationOrientation;


    QSet<int64_t> getZoneBorders(glm::ivec2 zoneCoord);
...
public:
    ...
    void drawInitial(ShaderProgram *shaderProgram);

    // Initializes the Chunks that store the 64 x 256 x 64 block scene you
    // see when the base code is run.
    void CreateTestScene();
    BlockType getBlockType(int height, bool isTop);

    void updateTerrainBasedOnPlayerPosition(const glm::vec3 player_position);

    void multiTheadedWorker(glm::vec3 playerPos, glm::vec3 playerPrevPos, float dT);

    void tryExpansion(glm::vec3 playerPos, glm::vec3 playerPrevPos);

    // void spawnFBMWoker(int chunkX, int chunkZ);
    void spawnFBMWoker(int64_t zoneKey);
    void spawnFBMWorkers(const QSet<int64_t> &zonesToGenerate);
    void spawnVBOWorker(Chunk* chunkNeedingVBOData);
    void spawnVBOWorkers(const std::unordered_set<Chunk*> &chunksNeedingVBOData);
    void checkThreadResult();

    void generateRiver(int chunkX, int chunkZ, std::vector<glm::ivec2> &river);
